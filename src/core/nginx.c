...
main(int argc, char *const *argv)
{
	...
	// 初始化 init_cycle->log,对于信号处理及选项处理是必要的
	...
	// @:https://blog.csdn.net/xhjcehust/article/details/75209347
    if (ngx_add_inherited_sockets(&init_cycle) != NGX_OK) {
        return 1;
    }
	
	// 预初始化模块
	// 记录模块在ngx_modules数组中的小标作为索引
	// 记录模块名
...
ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
	...
	// 获取nginx 环境变量
	// NGINX_VAR 中存储了老旧nginx进程打开的 监听Socket 文件描述符
	// 示例:NGINX_VAR="1456:1234:12346:8975"
    inherited = (u_char *) getenv(NGINX_VAR);

	// 没有获取到，说明没有旧nginx进程存在，返回OK
    if (inherited == NULL) {
        return NGX_OK;
    }
